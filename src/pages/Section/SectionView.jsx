...
export function SectionView({section, handleSectionClicked}) {
    ...
    let image;
    fetch('http://193.168.146.103:3000/images/2').then((response) => response.json()).then((data) => image = data);
    return (
        <div className={'section-view'}>
            <div className={'scenario'}>
                <p className={'text-scenario'}>{texte}</p>
                <img src={image != null ? image : '/src/assets/images/placeholder.png'} alt={'Section image'}/>
            </div>
            <MultipleChoiceComponent sections={sections} handleSectionClicked={() => handleSectionClicked(sections)}/>
        </div>
    );
}
